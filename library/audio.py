################################################################################################################
# audio.py      Version 2.3         11-Nov-2016       Chris Benson and Bill Manaris
#
###########################################################################
#
...
# REVISIONS:
#
#   2.3     11-Nov-2016 (cb) Fixed bug with multiple copy population of the choose input and output devices drop down menus.
#                            Cleaned up the code and created labels to issue warning about not having available input or output devices.
#
#   2.2     25-Oct-2016 (cb) Implemented JythonMusic version of JSyn's LineOut module for MultiChannel outputs. With
#                       this update both AudioSample and LiveSample can be played on Multi-Channel speaker arrays.
#
...
class jSyn_AudioEngine():
   ...
   def __init__(self):
      ...
      if jSyn_AudioEngine.instance == None:  # first time?
         ...
         self.inputPortID = -1                   # initialize to -1 so if user does not have audio inputs (ex. mic) avaiable on audio card the audio synth will start with the default input ID
         self.numberInputs = 0                   # initialize to 0 so if user does not have audio inputs (ex. mic) available on audio card the audio synth will start without any input ports
         self.outputPortID = -1                  # initialize to -1 so if user does not have audio outputs (ex. speakers) avaiable on audio card the audio synth will start with the default output ID
         self.numberOutputs = 0                  # initialize to 0 so if user does not have audio outputs (ex. speakers) available on audio card the audio synth will start without any output ports

         # ask user to pick which input and output devices (if any) they wish to use
         self.setAudioDevices()

         if self.inputDevices != []:
            print "Selected input device:", self.chosenInputDevice
         ...
         if self.outputDevices != []:
            print "Selected output device:", self.chosenOutputDevice
   ...
   def setAudioDevices(self):
      """Allows user to see what I/O devices are connected, and to select one of each (to use)."""
   ...
   def selectInputLine(self):
      """Create display with available audio input devices (if any).  Outputs error message if none."""

      self.inputDevices = []       # holds names and info for all available input devices
      ...
      for deviceID in range(self.deviceCount):
         ...
         if  numInputs > 0:
            ...
            # yes, so create menu item for this device...
            # (menu item consists of device name - as returned by system - and number of channels)
            menuItem = str(self.audioDeviceManager.getDeviceName(deviceID)) + "  (" + str(numInputs) + " input channels)"
            deviceDict = {'Name':menuItem, 'NumInputs':numInputs, 'DeviceID':deviceID}    # store info for this device
            self.inputDevices.append(deviceDict)                                          # and remember it along with other found devices

      # now, all available input devices are stored in self.inputDevices

      # create selection GUI
      self.displayInput = Display("Select Input Device", 500, 125) # display info to user

       # if there are no available audio input devices create error message for the user
      if self.inputDevices == []:
         self.displayInput.drawLabel("You Have No Devices That Are Capable Of Recording Audio", 45, 30)
         self.waitingToSetupInput = False # system waits for setup of input devices to finish before continuing setup of audio synthesizer
   ...
   def getInputDevice(self, selectedItem):
      ...
      # get key of selected input device
   ...
   def selectOutputLine(self):
      """Create display with available audio output devices (if any).  Outputs error message if none."""

      self.outputDevices = []      # holds names and info for all available output devices

      # find all audio output devices
      for deviceID in range(self.deviceCount):
         ...
         if  numOutputs > 0:
            ...
            deviceDict = {'Name':menuItem, 'NumOutputs':numOutputs, 'DeviceID':deviceID}  # store info for this device
            self.outputDevices.append(deviceDict)                                         # and remember it along with other found devices

      # now, only available output devices are stored in self.outputDevices
      self.displayOutput = Display("Select Output Device", 500, 125) # display info to user

       # if there are no available audio output devices create error message for the user
      if self.outputDevices == []:
          self.displayOutput.drawLabel("You Have No Devices That Are Capable Of Playing Audio", 45, 30)
          self.waitingToSetupOutput = False # system waits for setup of output devices to finish before continuing setup of audio synthesizer
...
################################################### LineOut ############################################################
# New LineOut class which allows using as many output channels as available (original jSyn LineOut supports only 2).
# It overloads the LineOut class built into Jsyn.
#
# It uses the same API as the default jSyn LineOut class (i.e., you can connect jSyn units (e.g., Delay, Pan) the same way)

from com.jsyn.unitgen import UnitGenerator, UnitSink
from com.jsyn.ports import UnitInputPort
from array import array   # using arrays instead of lists (for more speed / efficiency) to copy and add the incoming input data with the output buffer of jSyn_AudioEngine

class LineOut( UnitGenerator, UnitSink ):
   """Overloads default jSyn LineOut class.  Provides access to as many output channels available (original jSyn LineOut supports only 2)."""

   def __init__(self):

      self.numberOfChannels = jSyn_AudioEngine.instance.numberOutputs   # remember how many output channels are available
      self.inputPort = UnitInputPort(self.numberOfChannels,"Input")     # create input port with enough connections as output channels
      self.addPort(self.inputPort)                                      # expose it (i.e., make input port connectable)

      self.inputDataBuffers = []       # holds input buffers (one for each input port connection)
      self.outputDataBuffers = []      # holds output buffers (one for each output channel)

      # create buffers for both input and output
      for i in range(self.numberOfChannels):

         # create an input buffer and store it
         inputBuff = array('d')                    # (again, we are using arrays for efficiency)
         self.inputDataBuffers.append(inputBuff)   # store this input buffer

         # create an output buffer and store it
         outputBuff = array('d')
         self.outputDataBuffers.append(outputBuff)


   def generate(self, start, limit):
      """Overloading function needed by jSyn to update input and output buffers."""

      # update buffers
      for i in range(self.numberOfChannels):
         self.inputDataBuffers[i] = self.inputPort.getValues(i)                # get current values from input port and store them to corresponding buffer
         self.outputDataBuffers[i] = self.synthesisEngine.getOutputBuffer(i)   # get outgoing values from the synthesisEngine

      # add incoming frames to output buffers
      for frame in range(start, limit):
         for port in range(self.numberOfChannels):
            self.outputDataBuffers[port][frame] += self.inputDataBuffers[port][frame]   # make incoming data audible

   def getInput(self):
      """Overloading function needed by jSyn to get access to input port."""
      return self.inputPort

   def getNumChannels(self):
      """Returns number of output channels are available (used by application code)."""
...
class AudioSample():
   ...
   def __init__(self, filename, pitch=A4, volume=127):
      ...
      self.numOutputChannels = self.lineOut.getNumChannels() # Number of output channels the sample can utilize
      ...
      for i in range(self.lineOut.getNumChannels()):
          self.panLeft.output.connect( 0, self.lineOut.input, i )
      ...
      def __init__(self, maxSizeInSeconds = 30, pitch = A4, volume = 127, channels = 2): # SampleLength in milliseconds
         ...
         if jSyn.inputDevices != []: #
             ...
             self.setVolume(self.volume) # sets the desired volume
              ...
              # now, connect pan control to mixer for every available channel
             for i in range(self.lineOut.getNumChannels()):
                 self.panLeft.output.connect( 0, self.lineOut.input, i )
                 self.panRight.output.connect( 1, self.lineOut.input, i)
         ...
         else: # No audio input device detected
             print "You Have No Devices That Are Capable Of Recording Audio, You Will Not Be Able To Use LiveSample."
