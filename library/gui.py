...
#      #self.contentPane.repaint()
   ...
   def move(self, item, x, y):
      ...
      """
      if type(item) is Drawing:
          #item.setPosition(item.getX(),item.getY())

          for i in item.shapeList:
              newX = i.getX()
              newY = i.getY()
              self.reposition(i,newX,newY)
      else:
...
# Drawing
#
# Class for creating multi-shaped pictures that can be grouped as one giant shape
#
# Methods:
#
# CreateDrawing()
...
#
# addShape(addedShape)
#   Adds a created shape to the Drawing. Shapes include all drawable shapes as
#   well as Icons and pictures.
###############################################################################

class Drawing(JPanel, Widget, Drawable):

    def __init__(self):
        """
        Create new drawing
        """
        self.shapeList = []     # A list of shapes that are apart of the drawing
        self.drawingCenterX = None    # X coordinate of drawing (as a whole)
        self.drawingCenterY = None    # Y coordinate of drawing (as a whole)
        self.position = (0,0)
        self.color = None           # Color of whole drawing (initially each piece of drawing is its original color)
        self.offset = (0,0)

    def drawingCenter(self):
        """
        Finds center X and Y of drawing (as a whole)
        """
        if(len(self.shapeList) > 0):
            leastX = self.drawingCenterX
            greatestX = self.drawingCenterX
            leastY = self.drawingCenterY
            greatestY = self.drawingCenterY

            if(len(self.shapeList) == 1):
                leastX = self.shapeList[0].getX()
                greatestX = leastX
                leastY = self.shapeList[0].getY()
                greatestY = leastY
                ...
                for i in self.shapeList:
                    ...
                    if tempX < leastX:
                        leastX = tempX
                    elif tempX > greatestX:
                        greatestX = tempX

                    if tempY < leastY:
                        leastY = tempY
                    elif tempY > greatestY:
                        greatestY = tempY

            self.drawingCenterX = (leastX + greatestX) / 2.0
            self.drawingCenterY = (leastY + greatestY) / 2.0
            self.position = (self.drawingCenterX, self.drawingCenterY)

    def addObject(self, item):
        """
        Adds object to drawing as a composite of objects
        """
        self.shapeList.append(item)
        self.drawingCenter()

    def removeObject(self, item):
        """
        Removes object from drawing
        """
        self.shapeList.remove(item)
        self.drawingCenter()
    ...
    def encloses(self, widget):
        ...
        """
        for i in self.shapeList:
            if i.getBounds().contains( widget.getBounds() ):
                return True
        return False
    ...
    def intersects(self, widget):
        ...
        """
        for i in self.shapeList:
            if i.getBounds().intersects( widget.getBounds() ):
                return True
        return False
    ...
    def setPosition(self, x, y):
        ...
        Set the position of this Widget
        """
        offsetX = x - self.drawingCenterX
        offsetY = y - self.drawingCenterY

        for i in self.shapeList:
            i.setX(i.getX() + offsetX)
            i.setY(i.getY() + offsetY)

        self.drawingCenter() # drawingCenter sets the drawings center x and y values as well as its position

        #if this widget is already on a display, also reposition it
        #if self.display:
        #    for i in self.shapeList():
        #        self.display.move(i, i.getX(), i.getY())

    def getColor(self):
        """
        Returns list of each object's color in the drawing
        """
        colorList = []
        for i in self.shapeList:
            colorList.append(i.getColor())
    ...
    def setColor(self, color = None):
        ...
        if color == None:
            color = JColorChooser().showDialog(None, "Select a color", Color.ORANGE)
            print color   # useful side-effect for discovering new colors

        for i in self.shapeList:
            i.setColor(color)

    def setIndividualColor(self, specificPiece, color = None):
        """
        Change the color of one piece of the drawing. If no color provided, use dialog box to select.
        """
        if color == None:
            color = JColorChooser().showDialog(None, "Select a color", Color.ORANGE)
            print color

        self.shapeList[self.shapeList.index(specificPiece)].setColor(color)
...
if __name__ == "__main__":
    d = Display("blah", 500, 500)

    c = Circle(100,100,25, Color.GREEN)
    f = Circle(130, 100, 25, Color.ORANGE)
    e = Circle(160, 100, 25, Color.BLACK)

    #q = Circle(100,100,25, Color.GREEN)
    #r = Circle(130, 100, 25, Color.ORANGE)
    #s = Circle(160, 100, 25, Color.BLACK)

    endA = Circle(170,200,25)
    endB = Circle(200,200,25)
    endC = Circle(230,200,25)
...
### FIFTH DISPLAY TEST ###
   ...
   class Ball(Circle):
      ...
      def clicked(self, x, y):
         """"Executed when the ball is clicked with the mouse (x, y are the mouse coordinates).For now, we make the ball (randomly) jump in the air, and play sound.""""
