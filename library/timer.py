...
class EnvelopeTimer:
   """It calls a provided function giving it specified values at specified times (in milliseconds).
      ...
      EnvelopeTimer(function, values, times)
      
      For example, given an AudioSample a, 
      
      EnvelopeTimer(function=a.setVolume, values=[0, 50, 127], times=[0, 10, 105])
   ...
   def __init__(self, function, values, times):
      ...
      # create necessary timers for each value / function call
      self.timers = []     # hold necessary timers (one per function call)
      for i in range( self.numEnvelopePoints ):
         self.timers = Timer2( self.envelopeTimes[i], self.function, [self.envelopeValues[i]], False)
      # now, all necessary timers have been created
      
      # remember if we are paused
      self.hasPaused = False

      # remember all timers created (so that they can be stopped/terminated by JEM, if desired)
      for timer in self.timers:
         __ActiveTimers__.append( timer )

   def start(self):
      """(Re)start EnvelopeTimer to begin/continue calling function."""
      for timer in self.timers:
         timer.start()
   ...
   def isRunning(self):
      ...
      isRunning = False    # initialize
      for timer in self.timers:
         isRunning = isRunning or timer.isRunning()   # as long as one timer is running, we are too!
...
# this version below was a bit over-engineered... new version is simpler and hopefully more robust
class EnvelopeTimerOLD:
